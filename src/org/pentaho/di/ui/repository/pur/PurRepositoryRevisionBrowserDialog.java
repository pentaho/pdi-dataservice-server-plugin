	...
	public String open()
	{
		try {
	        ...
	        ColumnInfo[] colinf = new ColumnInfo[] {
	        		new ColumnInfo(BaseMessages.getString(PKG, "PurRepositoryRevisionBrowserDialog.Columns.Revision.Name"), ColumnInfo.COLUMN_TYPE_TEXT, false, true),
	        		new ColumnInfo(BaseMessages.getString(PKG, "PurRepositoryRevisionBrowserDialog.Columns.Revision.Comment"), ColumnInfo.COLUMN_TYPE_TEXT, false, true),
	        		new ColumnInfo(BaseMessages.getString(PKG, "PurRepositoryRevisionBrowserDialog.Columns.Revision.Date"), ColumnInfo.COLUMN_TYPE_TEXT, false, true),
	        		new ColumnInfo(BaseMessages.getString(PKG, "PurRepositoryRevisionBrowserDialog.Columns.Revision.Login"), ColumnInfo.COLUMN_TYPE_TEXT, false, true),
	        	};
			...
			lsOpen       = new Listener() { public void handleEvent(Event e) { ok();     } };
			wOpen.addListener    (SWT.Selection, lsOpen    );
	
			wClose=new Button(shell, SWT.PUSH);
			wClose.setText(BaseMessages.getString("System.Button.Close")); //$NON-NLS-1$
			lsClose   = new Listener() { public void handleEvent(Event e) { cancel(); } };
	...
	public void getData()
	{
		...
		for (ObjectRevision revision : revisions) {
			...
			wFields.add(
					revision.getName(), 
					revision.getComment(), 
					XMLHandler.date2string(revision.getCreationDate()), 
					revision.getLogin()
				);
		}
