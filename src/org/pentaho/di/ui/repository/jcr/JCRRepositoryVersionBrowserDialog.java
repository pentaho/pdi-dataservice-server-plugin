...
public class JCRRepositoryVersionBrowserDialog implements RepositoryVersionBrowserDialogInterface {
	private static Class<?> PKG = JCRRepositoryVersionBrowserDialog.class; // for i18n purposes, needed by Translator2!!   $NON-NLS-1$
	...
	public JCRRepositoryVersionBrowserDialog(Shell parent, int style, Repository repository, RepositoryElementLocationInterface element)
	{
	...
	public String open()
	{
		try {
			...
			versions = repository.getVersions(element);
	...
	public void getData()
	{
		...
		for (ObjectVersion version : versions) {
			...
			wFields.add(
					version.getName(), 
					version.getComment(), 
					XMLHandler.date2string(version.getCreationDate()), 
					version.getLogin()
				);
		}
