...
public class DataServiceMetaStoreUtil extends MetaStoreUtil {
  ...
  public static IMetaStoreElement createOrUpdateDataServiceElement( IMetaStore metaStore,
  ...
  private static void populateDataServiceElement( IMetaStore metaStore, IMetaStoreElement element,
                                                  DataServiceMeta dataServiceMeta ) throws MetaStoreException {
    ...
    element.addChild( metaStore
      .newAttribute( DataServiceMeta.DATA_SERVICE_TRANSFORMATION_FILENAME, dataServiceMeta.getTransFilename() ) );
    element.addChild( metaStore
      .newAttribute( DataServiceMeta.DATA_SERVICE_TRANSFORMATION_REP_PATH, dataServiceMeta.getTransRepositoryPath() ) );
    element.addChild( metaStore
      .newAttribute( DataServiceMeta.DATA_SERVICE_TRANSFORMATION_REP_OBJECT_ID, dataServiceMeta.getTransObjectId() ) );
    element.addChild( metaStore
      .newAttribute( DataServiceMeta.DATA_SERVICE_TRANSFORMATION_REP_OBJECT_ID, dataServiceMeta.getTransObjectId() ) );

  }

  public static DataServiceMeta loadDataService( IMetaStore metaStore, String dataServiceName, DataServiceMeta meta )
    throws MetaStoreException {
    IMetaStoreElementType elementType = createDataServiceElementTypeIfNeeded( metaStore );
  ...
  private static DataServiceMeta elementToDataService( IMetaStoreElement element, DataServiceMeta meta ) {
    ...
    meta.setTransObjectId(
      StringUtils.isEmpty( transObjectIdString ) ? null : new StringObjectId( transObjectIdString ) );
    meta.setCacheMaxAgeMinutes(
      NumberUtils.toInt( getChildString( element, DataServiceMeta.DATA_SERVICE_CACHE_MAX_AGE_MINUTES ), 0 ) );
    meta.setCacheMethod(
      ServiceCacheMethod.getMethodByName( getChildString( element, DataServiceMeta.DATA_SERVICE_CACHE_METHOD ) ) );
    return meta;
  }
