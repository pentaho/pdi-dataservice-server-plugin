...
/**
 * This class describes a job checkpoint logging table
 * 
 * @author matt
 *
 */
@LogTablePlugin(
    id="CheckpointLogTable",
    name="Checkpoint log table",
    description="The logging table which will keep track of which check-point job entry was reached in a previous execution"
   )
public class CheckpointLogTable extends BaseLogTable implements Cloneable, LogTablePluginInterface {
  ...
  public CheckpointLogTable() {
    super(new Variables(), null, null, null, null);
  }
	...
	@Override
	public void setContext(VariableSpace space, HasDatabasesInterface databasesInterface) {
	  super.space = space;
	  super.databasesInterface = databasesInterface;
	  addDefaultFields(this);	  
	}
	
	@Override
	public String getLogTablePluginUIClassname() {
	  return CheckpointLogTableUserInterface.class.getName();
	}
	
  @Override
  public TableType getType() {
    return TableType.JOB;
  }
  ...
  public void replaceMeta(LogTableCoreInterface logTableInterface) {
    if (!(logTableInterface instanceof CheckpointLogTable)) return;
    
    CheckpointLogTable logTable = (CheckpointLogTable) logTableInterface; 
    super.replaceMeta(logTable);
    maxNrRetries = logTable.maxNrRetries;
    runRetryPeriod = logTable.runRetryPeriod;
    namespaceParameter = logTable.namespaceParameter;
    saveParameters = logTable.saveParameters;
    saveResultRows = logTable.saveResultRows;
    saveResultFiles = logTable.saveResultFiles;
  }
	...
	private static void addDefaultFields(CheckpointLogTable table) {
  ...
  public LogTableField getParameterXmlField() {
	...
	public RowMetaAndData getLogRecord(LogStatus status, Object subject, Object parent) throws KettleException {
		if ((parent==null || (parent instanceof Job)) && (subject==null || (subject instanceof Result))) {
			...
			for (LogTableField field : fields) {
				if (field.isEnabled()) {
					...
					if (job!=null) {
					  ...
					  int attemptNr = JobRestartConst.getCheckpointAttemptNr(job);
					  Date runStartDate = JobRestartConst.getCheckpointRunStartDate(job);
            
						switch(ID.valueOf(field.getId())) {
            case ID_JOB_RUN : value = new Long(runId); break;
            ...
            case CHECKPOINT_NAME : value = checkpointJobEntry==null ? null : checkpointJobEntry.getName(); break;
            case CHECKPOINT_COPYNR : value = checkpointJobEntry==null ? null : Long.valueOf(checkpointJobEntry.getNr()); break;
            case ATTEMPT_NR: value = Long.valueOf( attemptNr ); break;
